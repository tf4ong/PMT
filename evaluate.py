...
def main(dic_analysis,dic_train):
    INPUT_SIZE = dic_train['size']
    STRIDES, ANCHORS, NUM_CLASS, XYSCALE = utils.load_config(dic_train['model'],dic_train['classes'])
    CLASSES = dic_train['classes']
    ...
    # Build Model
    saved_model_loaded = tf.saved_model.load(dic_analysis['weightpath'], tags=[tag_constants.SERVING])
    infer = saved_model_loaded.signatures['serving_default']

    num_lines = sum(1 for line in open(FLAGS.annotation_path)) # check the format of val
